...
/// Gets the directory path for an object.
pub fn get_directory_path_for_object(
    directory_layout: DirectoryLayout,
    object_path: Option<&Path>,
    object: &Object,
) -> Option<String> {
    let format = object.file_format();
    let ty = match (object.kind(), format) {
        (ObjectKind::Debug, FileFormat::MachO) => FileType::MachDebug,
        (_, FileFormat::MachO) => FileType::MachCode,
        (_, FileFormat::Pdb) => FileType::Pdb,
        (_, FileFormat::Pe) => FileType::Pe,
        (ObjectKind::Debug, FileFormat::Elf) => FileType::ElfDebug,
        (_, FileFormat::Elf) => FileType::ElfCode,
        (_, FileFormat::Breakpad) => FileType::Breakpad,
        (_, FileFormat::Unknown) => return None,
    };
    let object_id = ObjectId {
        code_id: object.code_id(),
        code_file: object_path.map(|x| x.display().to_string()),
        debug_id: Some(object.debug_id()),
        debug_file: object_path.map(|x| x.display().to_string()),
    };
    get_directory_path(directory_layout, ty, &object_id)
}
