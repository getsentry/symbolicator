...
impl GcsState {
    ...
    async fn exists(&self, cache_name: CacheName, key: &str) -> Result<bool, CacheError> {
        let token = self.get_token().await?;
        let url =
...
struct UploadMessage {
    /// The cache name, for metrics purposes.
    cache_name: CacheName,
...
impl SharedCacheService {
    ...
    async fn single_uploader(
        ...
        sentry::configure_scope(|scope| {
            ...
            map.insert("cache".to_string(), cache_name.as_ref().into());
        ...
        let res = match *backend {
            SharedCacheBackend::Gcs(ref state) => {
                state.store(cache_name, &key, content, reason).await
            }
...
mod tests {
    ...
    async fn test_filesystem_fetch_found() {
        ...
        let key = String::from("objects/0/global/some_item");
    ...
    async fn test_gcs_fetch_not_found() {
        ...
        let key = format!("objects/0/{}/some_item", Uuid::new_v4());
