...
use crate::settings::EventArgs;

use self::remote::EventKey;

pub fn process_event(args: EventArgs) -> anyhow::Result<()> {
    unimplemented!()
        ...
        match self {
            Payload::Event(event) => {
                let symbolication_request =
                    event_json::create_symbolication_request(scope, sources, event)
