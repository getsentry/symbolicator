...
async fn test_deny_list() {
    ...
    assert_eq!(
        ...
        (
            ...
            ObjectUseInfo::None,
            ObjectDownloadInfo::Error {
                details: "download failed: Host localhost is temporarily blocked because there were too many download failures. It will remain blocked for a maximum of 500ms. The error that triggered the block was: `download timed out after 200ms`.".into()
            }
        )
    );
