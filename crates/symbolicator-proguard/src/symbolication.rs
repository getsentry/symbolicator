...
mod tests {
    ...
    fn map_frames_via_stacktrace(
        ...
        let mut stats = SymbolicationStats::default();
        ProguardService::map_stacktrace(
            mappers,
...
io.sentry.sample.MainActivity -> io.sentry.sample.MainActivity:
    ...
    1:1:void onClickHandler(android.view.View):40 -> t";
        ...
        let mapped_frames = map_frames_via_stacktrace(&[&cache], frames.to_vec(), None);
...
";
        ...
        let mapped_frames =
            map_frames_via_stacktrace(&[&cache], frames.to_vec(), Some("org.slf4j"));
    ...
    fn doesnt_set_in_app_if_not_resolved() {
        ...
        let remapped = map_frames_via_stacktrace(&[], vec![frame.clone()], Some("android"));
...
"#;
        ...
        let mapped_frames = map_frames_via_stacktrace(&[&cache], vec![frame.clone()], None);
...
io.wzieba.r8fullmoderenamessources.R -> a.d:
    void <init>() -> <init>
      # {"id":"com.android.tools.r8.synthesized"}"#;
        ...
        let remapped_frames = map_frames_via_stacktrace(&[&cache], frames.clone(), None);
        let (remapped_filenames, remapped_abs_paths): (Vec<_>, Vec<_>) = remapped_frames
            .into_iter()
...
"#;
        ...
        let remapped = map_frames_via_stacktrace(&[&cache], frames, None);
    ...
    fn remap_outline_complex() {
        ...
        let remapped_frames = map_frames_via_stacktrace(&[&cache], frames, None);
