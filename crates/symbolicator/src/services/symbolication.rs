...
impl TempSymbolProvider {
    ...
    /// This reads the CFI caches from disk and returns them in a format suitable for the
    /// breakpad processor to stackwalk.
    pub fn new<'a, M: Iterator<Item = (&'a DebugId, &'a PathBuf)>>(modules: M) -> Self {
...
fn stackwalk_with_rust_minidump(
    ...
    let modules: BTreeMap<DebugId, RawObjectInfo> = process_state
        .modules
        .iter()
        .map(|module| {
            (
    ...
    let mut missing_modules = modules.clone();
    missing_modules.retain(|id, _| provider.missing_ids().contains(id));
